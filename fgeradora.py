...
TRATAMENTOS = ['Consulta_rotina', 'Vacinação', 'Castração', 'Limpeza_tártaro', 'Hemograma', 'Ultrassonografia', 'Curativo_ferida']
...
def gerar_pets(identidade):
    ...
    tratamentos_qtd = random.randint(1, 3)
    tratamentos_lista = random.sample(TRATAMENTOS, tratamentos_qtd) # sample() é o mesmo que choice, mas ao invés de um elemento único, n elementos únicos dentro da lista.
    tratamentos = str(tratamentos_lista)

    return f"{identidade};{nome};{especie};{idade};{peso};{tratamentos};{cadastro}\n" # alterado de "," para ";".
...
def gerar_arquivo_dados(nome_arquivo, qtd_linhas):
    ...
    with open(nome_arquivo, "w", encoding="utf-8") as file:
        ...
        for i in range(1, qtd_linhas + 1): # aqui imprimimos pacientes aleátórios conforme a quantidade de linhas que definimos pros tamanhos dos arquivos.
            ...
            file.write(dados_por_linha) # aqui retorna a função anterior escrita no arquivo.
...
# DEBUG - resultados do meu PC: ("\" pra Windows, "/" pra Linux)
gerar_arquivo_dados("data\pequeno.txt", 100)      # 100 pacientes (0.0008 segundos)
gerar_arquivo_dados("data\medio.txt", 10000)      # 10 mil pacientes (0.0488 segundos)
gerar_arquivo_dados("data\grande.txt", 100000)    # 100 mil pacientes (0.4699 segundos)
gerar_arquivo_dados("data\gigante.txt", 1000000)  # 1 milhão de pacientes (4.9325 segundos)
